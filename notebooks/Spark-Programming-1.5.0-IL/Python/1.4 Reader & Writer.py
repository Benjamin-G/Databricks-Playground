...
display(productsDF2.orderBy(productsDF2.price.desc()).take(5))
display(productsDF2.groupBy('name').sum('price').withColumnRenamed("sum(price)", "sum").orderBy('sum', ascending=False) )
display(productsDF2.groupBy('item_id').sum('price').withColumnRenamed("sum(price)", "sum").orderBy('sum', ascending=False) )
...
# TODO
print(productsDF.schema)
display(productsDF)
productsOutputPath = workingDir + "/delta/products"
# productsDF.write.mode("overwrite").saveAsTable("productsDF")
(productsDF.write
  .format("delta")
  .mode("overwrite")
  .parquet(productsOutputPath))
...
print(dbutils.fs.ls(productsOutputPath))
