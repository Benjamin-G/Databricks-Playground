...
# TODO
topTrafficDF = (trafficDF.orderBy(trafficDF.total_rev.desc()).limit(3))
...
finalDF = (topTrafficDF
           .withColumn('avg_rev', (topTrafficDF.avg_rev * 100).cast('long') / 100)
           .withColumn('total_rev', (topTrafficDF.total_rev * 100).cast('long') / 100)
)
...
# TODO
from pyspark.sql.functions import round
bonusDF = ( topTrafficDF.select('traffic_source', round('avg_rev', 2).alias('avg_rev'), round('total_rev', 2).alias('total_rev') ) )
display(bonusDF)
...
chainDF = (df
           ...
           .agg(round(sum('revenue'),2).alias('total_rev'), round(avg('revenue'),2).alias('avg_rev'))
