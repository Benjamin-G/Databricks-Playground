...
# spark.conf.set("spark.sql.shuffle.partitions". sc.defaultParellelism)
...
eventsDF = (df
#             .withColumn('createdAt', ( col('event_timestamp') / pow(10,6) ).cast('timestamp') )
...
# TODO
from pyspark.sql.functions import window, col, approx_count_distinct, hour #, date_formate
...
#              .select('traffic_source', 'active_users', date_format('window.start', 'h').alias('hour'))
